...
**Everything these fields carry is read by someone who has never seen the plugin.** The `events:` prose lands in the hub's catalog beside every other plugin's, `suite:` becomes the doc site's card, and the CHANGELOG's top section becomes the release body — so a plugin's internal vocabulary reads as noise there. A requirement ID like `PROV-07` names nothing a catalog reader can resolve, and least of all at the aggregate surface, where someone reading one plugin's events page has no reason to know another's spec. Write these for that reader: what the subscription gets them, in their words.

The same reader is why `handled_by` and `emitted_by` name the **file** that handles or emits, not a symbol inside it. A file is something a reader can open, and a rename shows up in the diff; a function name drifts with nothing to catch it, since nothing here resolves either one.
