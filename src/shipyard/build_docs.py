...
#
# Each token that isn't docsify-themeable's own carries a fallback. A bare
# `var(--undefined)` inside the `border` shorthand takes the whole declaration
# down to `none`, so a missing token doesn't soften a pill: it erases it, and the
# marks read as loose words beside the title. The border falls back to the theme's
# own `--mono-tint2`; muted *text* goes straight to a literal, since the tint
# scale's light end is a border colour and reads as washed-out type.
HOME_STYLE = """<style>
...
.markdown-section .ph-tag{display:inline-flex;align-items:center;gap:7px;padding:3px 11px;
  border:1px solid var(--border-subtle,var(--mono-tint2,#e3e3e3));border-radius:20px;
  text-decoration:none;
  font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;font-size:12px;
  color:var(--muted-color,#767676);white-space:nowrap}
.markdown-section a.ph-tag:hover{color:var(--base-color);
  border-color:var(--muted-color,#767676);text-decoration:none}
...
.markdown-section a.ph-cli:hover{color:var(--theme-color);
  border-color:color-mix(in srgb,var(--theme-color) 64%,transparent)}
...
# The suite's interop key: `<reverse-domain>.<plugin>/<entity>.<event>`. The
# plugin half comes from `name:`, so a producer never restates its own prefix and
# cannot typo it.
EVENT_NAMESPACE = "codes.bridgeai"


def event_key(plugin_name: str, key: str) -> str:
    """The fully-qualified key a subscriber matches on."""
    return f"{EVENT_NAMESPACE}.{plugin_name}/{key}"


def _event_fields(declared: object) -> list[tuple[str, str, str]]:
    """(name, value-note, description) per declared field.

    A plain string is shorthand for a required string field, which is what most
    of them are — the same shorthand `author:` takes. The expanded form carries
    `describe`, and `optional` for a field a consumer must not rely on: a
    publisher that reads a value back from somewhere else can hand over an empty
    one, and a table that said "always set" would be lying about it."""
    out = []
    for name, spec in (declared or {}).items():
        if isinstance(spec, dict):
            describe = spec.get("describe", "")
            note = "may be empty" if spec.get("optional") else "always set"
            kind = spec.get("type", "string")
            if kind != "string":
                note = f"{note}, {kind}"
        else:
            describe, note = str(spec), "always set"
        out.append((str(name), note, describe))
    return out


def _declared_events(plugin: dict) -> list[dict]:
    """The events this plugin publishes, from plugin.yml's `events.publishes`."""
    return list((plugin.get("events") or {}).get("publishes") or [])


def _render_events_md(plugin: dict) -> str:
    """The events page: a section per published event, each anchored so a
    subscriber can be pointed at one event rather than at the page.

    The anchors are explicit because a dotted heading does not slugify to
    anything a reader would guess — docsify strips punctuation rather than
    converting it, so `cr.created` becomes `crcreated`."""
    name = plugin["name"]
    events = _declared_events(plugin)
    first = events[0]["key"]
    sample = event_key(name, first)

    out = ["<!-- Generated by shipyard build-docs from plugin.yml. Do not edit. -->", "",
           "# Events", "",
           f"`{name}` announces facts on its own stdout, so another plugin can react to "
           "them without it knowing that plugin exists. Nothing here is read back, and a "
           "session where nothing is listening pays nothing for it.", "",
           "An announcement is one line: a key, then a compact JSON object.", "",
           "```text", f'{sample} {{"uri":"https://github.com/o/r/pull/3533"}}', "```", ""]

    if len(events) > 1:
        out += ["The events, each linkable on its own:", ""]
        out += [f"- [`{e['key']}`](#{e['key'].replace('.', '-')})" for e in events]
        out += [""]

    for event in events:
        key = event["key"]
        out += [f"## `{key}` :id={key.replace('.', '-')}", "",
                "```text", event_key(name, key), "```", ""]
        if event.get("when"):
            out += [_flow(event["when"]), ""]
        fields = _event_fields(event.get("fields"))
        if fields:
            out += ["| Field | Value | Meaning |", "|---|---|---|"]
            out += [f"| `{f}` | {note} | {_cell(desc)} |" for f, note, desc in fields]
            out += [""]
        if event.get("emitted_by"):
            # A bare path reads as code; anything carrying a space is prose about
            # where the announcement comes from, and setting a sentence in code
            # type makes it look like a filename.
            emitted = _flow(event["emitted_by"])
            shown = emitted if " " in emitted else f"`{emitted}`"
            out += [f"Emitted by {shown}.", ""]
    ...
    out += ["## Reacting to one", "",
            ...
            "```bash",
            "input=$(cat)",
            "output=$(printf '%s' \"$input\" | jq -r '.tool_response.stdout // empty' "
            "2>/dev/null) || exit 0",
            "line=$(printf '%s' \"$output\" | grep -m1 "
            f"'^{sample.replace('.', chr(92) + '.')}[[:space:]]') || exit 0",
            "uri=$(printf '%s' \"${line#* }\" | jq -r '.uri // empty' 2>/dev/null) || exit 0",
            "```", "",
            f"Register that in your own plugin's `hooks.yml`. `{name}` gains no entry, no "
            "config, and no knowledge that you subscribed, which is what lets either side "
            "ship without the other.", ""]
    out += _alert("NOTE",
                  "**Exit 0 on every path**, a body that will not parse included. The hook "
                  "runs on every Bash call in every session where your plugin is enabled, so "
                  "one that errors or hangs taxes turns that have nothing to do with it.")
    out += _alert("WARNING",
                  "**A parsed value is not a safe value.** The line is guaranteed to occupy "
                  "one line; what you decode out of its JSON body is not. A `\\u` escape "
                  "decodes to whatever control character it names, so strip control "
                  "characters from any value before putting it on a terminal surface or into "
                  "the agent's context. Treat every value as text a stranger wrote.")
    out += ["## The rules", "",
            "The grammar, the reserved field names, and what else a subscriber owes are the "
            "suite's rather than this plugin's: "
            "[the plugin interop contract](https://github.com/chris-peterson/claude-marketplace/blob/main/authoring/plugin-contract.md).", ""]
    return "\n".join(out)


def _alert(kind: str, body: str) -> list[str]:
    """A docsify flexible-alerts callout, as one blockquote.

    The marker and the body have to sit in the *same* blockquote: a blank line
    between them ends the quote, so the page renders a literal `[!NOTE]` above an
    unrelated one."""
    return [f"> [!{kind}]", f"> {_flow(body)}", ""]


def _flow(text: str) -> str:
    """A declared prose block as one paragraph. YAML folding already collapsed
    its newlines; this drops the leftover run of spaces."""
...
def _curated_by_kind(cmds: list, describe: dict) -> dict:
    """Route each `cmds` description to the artifact kind it titles.

    A label's leading `/` separates the two things a plugin can name: an artifact
    a session invokes, or a CLI you run in your own shell. The two can share a
    name — beacon ships `/beacon:beacon` and `beacon` — so each kind gets its own
    slot, and a CLI's copy can only ever reach the marketplace card.

    Which kind a slashed label titles is read from `describe:` rather than
    inferred from the label: `/tack:start` is a skill and `/tack:install-tack` a
    command, and nothing in either label says so."""
    curated = {kind: {} for kind in ARTIFACT_HEADINGS}
    for entry in cmds:
        label, desc = entry[0], entry[1]
        if not label.startswith("/"):
            # A CLI. It titles no row here; the marketplace card is its reader.
            continue
        name = label.rsplit(":", 1)[-1] if ":" in label else label.lstrip("/")
        for kind, entries in describe.items():
            if kind in curated and name in (entries or {}):
                curated[kind][name] = desc
    return curated
...
def _render_tags(spec: dict, published: set[str]) -> str:
    """The row of marks a plugin's home page opens with: what it ships, and what
    version. Empty for a plugin with neither.

    Its own function because a hand-written home page wants the row without the
    generated page around it — `_tags.md` is the same row on its own, for a
    `[](_tags.md ':include')`. That keeps the version out of anyone's hands: it
    comes from `plugin.yml`, which the release bumps, so a page that states it
    can't fall behind the release."""
    ...
    if suite.get("cli"):
        ...
        # A reader who notices it is asking what the commands are, so where the
        # reference page exists the mark is the way there. The raw `#/` is
        # deliberate: docsify rewrites the links it finds in markdown, and this
        # is HTML it passes through untouched.
        route = links.route_of(pathlib.Path("."),
                               pathlib.Path(gen_cli_manifest.DOCS_PAGE))
        tags.append(f'<a class="ph-tag ph-cli" href="#{route}">cli</a>'
                    if route in published
                    else '<span class="ph-tag ph-cli">cli</span>')
    if spec.get("version"):
    ...
    return f'<p class="ph-tags">{"".join(tags)}</p>' if tags else ""


def _render_tags_md(spec: dict, published: set[str]) -> str:
    """The marks row as a page of its own, styles included, for a home page that
    is hand-written rather than the generated one."""
    return "\n".join([
        "<!-- Generated by shipyard build-docs from plugin.yml. Do not edit. -->", "",
        HOME_STYLE, "", _render_tags(spec, published), ""])
...
def _render_home_md(spec: dict, r: pathlib.Path, published: set[str]) -> str:
    ...
    # `cmds` is the author's own copy for the artifacts a user reaches for, so it
    # wins over the description generated from the artifact's own frontmatter.
    curated = _curated_by_kind(suite.get("cmds") or [], describe)

    out = ["<!-- Generated by shipyard build-docs from plugin.yml. Do not edit. -->", "",
           HOME_STYLE, ""]

    if row := _render_tags(spec, published):
        out += [row, ""]
    ...
    for kind in ARTIFACT_HEADINGS:
        if describe.get(kind):
            out += _artifact_table(kind, describe[kind], curated[kind], name, pages, published)
...
def run(root: str | pathlib.Path | None = None) -> int:
    ...
    # Same shape as the hooks page: a surface this plugin exposes to other
    # software, rendered from the declaration rather than restated by hand.
    if _declared_events(plugin):
        docs.mkdir(parents=True, exist_ok=True)
        (docs / "events.md").write_text(_render_events_md(plugin))
    ...
    if plugin.get("suite"):
        ...
        if _render_tags(plugin, published):
            (docs / "_tags.md").write_text(_render_tags_md(plugin, published))
